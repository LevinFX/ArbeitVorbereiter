...
with st.sidebar:
    ...
    Thema = st.text_input("Wöfür willst du", key="thema_auswahl")
...
if "messages" not in st.session_state:
    ...
    if not klasse:
        ...
        if files:
            ...
            texts = raw['content']
            st.session_state["messages"] = [{"role": "system", "content": 'Du bist der beste Lehrer für das Fach '+Fach +'und einer deiner Schüler geht in die '+str(klasse)+'. Klasse eines Gymnasiums in Schleswig Holstein. Deshalb sollst du, falls es nicht vom Schüler anders verlangt wird, auf Deutsch reden (außer du bist Englischlehrer) Deine Aufgabe ist es nun, ihm Feedback zu geben. Der Schüler hat ein '+Thema+' gemacht. Gebe ihm konstruktive Kritik, lobe ihn und benote das Ergebnis. Frage ihn auch, ob du es korrigieren sollst. Hier villeicht noch ein paar Worte des Schülers zu dir: '+Prompt+'. Ende der Nachricht des Schülers, hier ist noch Text, welcher aus Dateien, die der Schüler dir gegeben hat, extrahiert wurde: '+texts}]
        else:
            st.session_state["messages"] = [{"role": "system", "content": 'Du bist der beste Lehrer für das Fach '+Fach +'und einer deiner Schüler geht in die '+str(klasse)+'. Klasse eines Gymnasiums in Schleswig Holstein. Deshalb sollst du, falls es nicht vom Schüler anders verlangt wird, auf Deutsch reden (außer du bist Englischlehrer) Deine Aufgabe ist es nun, ihm Feedback zu geben. Der Schüler hat ein '+Thema+' gemacht. Gebe ihm konstruktive Kritik, lobe ihn und benote das Ergebnis. Frage ihn auch, ob du es korrigieren sollst. Hier villeicht noch ein paar Worte des Schülers zu dir: '+Prompt}]
